...
import { getForkDecisionHistory, getInclusionStage, getLaymanTitle, getProposalPrefix } from '../utils';
...
const EipPage: React.FC = () => {
  ...
  const renderForkTimeline = (fork: ForkRelationship) => {
    const events = getForkDecisionHistory(eip as EIP, fork.forkName);
    ...
    return (
      <section key={fork.forkName} className="border border-slate-200 dark:border-slate-700 rounded-lg p-4 bg-white dark:bg-slate-800">
        ...
        {events.length === 0 ? (
          <p className="text-xs text-slate-500 dark:text-slate-400">
            No call-linked inclusion decisions have been recorded for this fork yet.
          </p>
        ) : (
          <ol className="relative border-l border-slate-200 dark:border-slate-700 ml-2 mt-2">
            {events.map((event, index) => {
              const isLatest = index === events.length - 1;
              const callLabel = `${event.callType.toUpperCase()} #${event.callNumber}`;

              return (
                <li key={`${fork.forkName}-${event.callPath}-${index}`} className="mb-4 ml-4">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                    ...
                    <Link
                      ...
                      onClick={() => handleExternalLinkClick('call', `/calls/${event.callPath}`)}
                    >
                      {callLabel}
                    </Link>
                    {event.callDate && ` on ${event.callDate}`}
                  </p>
                  {event.reason && (
